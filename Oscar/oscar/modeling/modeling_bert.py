...
class BertImgModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.goal_embeddings = nn.Linear(2, self.config.hidden_size, bias=True)
        self.goal_dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, goal=None,
        ...
        if img_feats is not None:
            if self.img_feature_type == 'dis_code':
                ...
                if img_feats.shape[2] < 2054:
                    img_feats = torch.cat((img_feats, torch.zeros(img_feats.shape[0], img_feats.shape[1], 2054-1030).cuda()), 2)
            ...
            if DEBUG:
                ...
                print("embedding_output", embedding_output.shape, "img_embedding", img_embedding_output.shape)
            embedding_output = torch.cat((embedding_output, img_embedding_output), 1)

        if goal is not None:
            goal_embedding_output = self.goal_embeddings(goal)
            goal_embedding_output = self.goal_dropout(goal_embedding_output)
            embedding_output = torch.cat((embedding_output, goal_embedding_output), 1)
            if DEBUG:
                print("embedding_output", embedding_output.shape, "goal_embedding", goal_embedding_output.shape)
        ...
        if LATE:
            if DEBUG:
                print("sequence_output", sequence_output.shape, goal_embedding_output.shape)
            pooled_output = self.pooler(torch.cat((sequence_output, goal_embedding_output), 1))
        else:
...
class BertForImageCaptioning(CaptionPreTrainedModel):
    ...
    def encode_forward(self, input_ids, img_feats, attention_mask, masked_pos, goal, masked_ids=None, 
            token_type_ids=None, position_ids=None, head_mask=None,
            is_training=True, encoder_history_states=None):
        outputs = self.bert(input_ids, img_feats=img_feats, attention_mask=attention_mask, goal=goal,
    ...
    def prepare_inputs_for_generation(self, curr_ids, past=None):
        ...
        if past is None:
            ...
            full_len = self.max_seq_len + self.od_labels_len + self.img_seq_len + 1
            ...
            if DEBUG:
                print("max_seq_len", curr_len, self.full_attention_mask.shape, attention_mask.shape)
    ...
    def generate(self, img_feats, attention_mask, masked_pos, goal, token_type_ids=None,
        ...
        self.goal = self._expand_for_beams(goal, num_expand)
        if DEBUG:
            print("full_masked_pos", self.full_masked_pos.shape)
        if not use_cbs:
            if num_beams > 1:
