...
### Modified by Leon
import sys
sys.path.append("/data2/zhongkai/VIP/vip/Oscar")
...
class CaptionTSVDataset(Dataset):
    def __init__(self, yaml_file, tokenizer=None, add_od_labels=True, add_cluster_labels=True,add_goals=True,
        ...
        self.cluster_file = find_file_path_in_yaml(self.cfg['cluster'], self.root)
        self.goal_file = find_file_path_in_yaml(self.cfg['goal'], self.root)
        ...
        self.cluster_tsv = None if not self.cluster_file else TSVFile(self.cluster_file)
        self.goal_tsv = None if not self.cluster_file else TSVFile(self.goal_file)
        ...
        self.add_cluster_labels = add_cluster_labels
    ...
    def get_od_labels(self, img_idx):
        ...
        if self.add_od_labels:
            ...
            # print("...", type(label_info), len(label_info))
            ### 
            od_labels = " ".join([l['class'] for l in label_info])
            # print(type(od_labels), od_labels)
        return od_labels

    def get_cluster_labels(self, img_idx):
        cluster_labels = None
        cluster_ = None
        if self.add_cluster_labels:
            ...
            if "turn" in cluster_labels or "pan" in cluster_labels:
                cluster_ += " turn"
            elif "forward" in cluster_labels or "straight" in cluster_labels:
                cluster_ += " forward"
            elif "stop" in cluster_labels:
                cluster_ += " stop"
    ...
    def __getitem__(self, idx):
        ...
        cluster_labels = self.get_cluster_labels(img_idx)
        goal = self.get_goals(img_idx)
        # print(goal, type(goal))
        # print("od_labels" ,od_labels, "cluster_labels", cluster_labels)
        if cluster_labels:
            # print(cluster_labels)
            example = self.tensorizer.tensorize_example(caption, features, text_b=od_labels+cluster_labels, goal=goal)
            # example = self.tensorizer.tensorize_example(caption, features, text_b=od_labels+cluster_labels, goal=[0,0])
            # example = self.tensorizer.tensorize_example(caption, features, text_b=od_labels, goal=goal)
            # example = self.tensorizer.tensorize_example(caption, features, text_b=od_labels, goal=[0, 0])
        else:
            example = self.tensorizer.tensorize_example(caption, features, text_b=od_labels, goal=goal)
...
class CaptionTensorizer(object):
    ...
    def tensorize_example(self, text_a, img_feat, text_b=None, goal=None,
        ...
        max_len = self.max_seq_len + self.max_img_seq_len + 1
        ...
        # full attention for C-G, L-G, R-G (Goal):
        attention_mask[:, -1] = 1
        attention_mask[-1, :] = 1
        ...
        goal  = torch.tensor([goal], dtype=torch.float) #+(2*torch.rand((1,2))-1)*50
        # print(goal.shape, torch.rand((1,2)).shape)

        if self.is_train:
            masked_ids = torch.tensor(masked_ids, dtype=torch.long)
            return (input_ids, attention_mask, segment_ids, img_feat, masked_pos, goal, masked_ids)
        return (input_ids, attention_mask, segment_ids, img_feat, masked_pos, goal)
        ...
        for step, (img_keys, batch) in enumerate(train_dataloader):
            ...
            if not args.scst:
                ...
                inputs = {'input_ids': batch[0], 'attention_mask': batch[1],
                    ...
                    'masked_pos': batch[4], 'goal': batch[5], 'masked_ids': batch[6]
                }
        ...
        with torch.no_grad():
            for step, (img_keys, batch) in tqdm(enumerate(test_dataloader)):
                ...
                inputs = {
                    'input_ids': batch[0], 'attention_mask': batch[1],
                    'token_type_ids': batch[2], 'img_feats': batch[3],
                    'masked_pos': batch[4], 'goal': batch[5],
                }
                if args.use_cbs:
                    inputs.update({
                        'fsm': batch[5],
